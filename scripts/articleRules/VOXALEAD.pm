...
sub extractArticle{
	...
	if ($response->is_success){
		...
		if ($isRedirected eq 'Advertisement'){
			...
			my $newLink  = $tree->findnodes_as_string(q{/html/head/a/href/});
			print " $newLink \n";
		...
		for  my $result ( $tree->findnodes(q{/html/body//div[@id="named-entities"]/ol/li})){					
					push @returnedText,  $result->getValue;
                }
		return \@returnedText ;
