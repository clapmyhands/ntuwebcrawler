...
sub extractArticle{
	...
	if ($response->is_success){
		...
		for  my $result ( $tree->findnodes(q{/html/body})){
	                $returnedText = $result->findvalue(q{//div[@class="text article"]}); # lock
                }

		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//div[@class="news_article"]}); # lock
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//div[@class="medium_txt"]}); 
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//p[@class="tcenter"]}); 
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
