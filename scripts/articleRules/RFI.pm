	...
	if ($response->is_success){
		...
		for  my $result ( $tree->findnodes(q{/html/body})){
			#print $result->findvalue(q{//div[@class="spacearticle"]});
	                ...
	                $returnedText = $result->findvalue(q{//div[@class="article-main-text"]});
			#print "\nt1\n $returnedText \n\n";
                }
		...
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//div[@class="article-header"]/h2[@class="article-header-intro"]});
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//p});
	                }
		}
		#print "$returnedText\n";
		return $returnedText ;
