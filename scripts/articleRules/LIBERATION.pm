...
sub extractArticle{
	...
	if ($response->is_success){
		...
		for  my $result ( $tree->findnodes(q{/html/body})){
	                $returnedText = $result->findvalue(q{//div[@itemprop="articleBody"]}); # lock
                }

		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//div[@class="object-content"]}); # lock
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//div[@class="entry-body"]}); # lock
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//p[@class="spip"]}); # lock
	                }
		}
		if ( $returnedText eq ''){
			print "empty string\n";
		...
		if ( $returnedText eq ''){
			print "empty string\n";
			for  my $result ( $tree->findnodes(q{/html/body})){
	                	$returnedText = $result->findvalue(q{//div[@class="articleTeaserGrey margeGauche toto"]});
	                }
		}
		#print "$returnedText\n";
		return $returnedText ;
